...
class NCRPNode(object):
    ...
    def __init__(self, num_levels, vocab, parent=None, level=0,
        ...
        if random_state is None:
            ### for same results RandomState(1) else RandomState()
...
class HierarchicalLDA(object):
    ...
    def dict_for_tree(self, n_words):
        dict_list = []
        self.helper_dict(self.root_node, n_words,dict_list)
        return dict_list

    def helper_dict(self, node,n_words,dict_list):
        parent = -1 if node.parent is None else node.parent.node_id
        dict_tree = {"topic_id": node.node_id,
                     "level": node.level,
                     "parent": parent,
                      "topic": node.get_top_words(n_words,with_weight=False),
                     'word_dist': node.word_counts}
        dict_list.append(dict_tree)
        for child in node.children:
            self.helper_dict(child,n_words,dict_list)
